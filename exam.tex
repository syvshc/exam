...
\end{enumerate} 
    ...
    \begin{enumerate}[resume=exer]
        ...
        \item  设 $ \symscr{A} $ 是实数域 $ \R $ 上 $ n\,(n\ge 1) $ 维线性空间 $ V $ 上的线性变换, 证明 $ \symscr{A} $ 至少有一个维数是 $ 1 $ 或 $ 2 $ 的不变子空间.
        ...
        \begin{enumerate}
            \item 对于任意两个不同的单位向量 $ \alpha, \beta \in V $, 总存在 $ V $ 上的镜面变换 $ \symscr{A} $, 使得 $ \symscr{A}(\alpha) = \beta $;
            \item 证明 $ V $ 上的任意正交变换 $ \symscr{B} $ 都可以表示成有限个镜面变换的乘积, 即存在 $ m $ 个镜面变换 $ \symscr{A}_{1}, \symscr{A}_{2}, \dots, \symscr{A}_{m} $, 使得
            \begin{align*}
                \symscr{B} = \symscr{A}_{1}\symscr{A}_{2}\dots\symscr{A}_{m}.
            \end{align*}
        \end{enumerate}
