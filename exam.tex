...
\def\sitem{%
  ...
  \expandafter\def\csname labelenum\romannumeral\@enumdepth\expandafter\endcsname\expandafter{%
    \expandafter\bfseries\expandafter\asteriskitem\originallabel}%
  \item
...
\begin{enumerate}
    ...
    \item \textbf{面积原理}
    \begin{enumerate}
        \item 设 $ f $ 是一个非负的递增函数, 则当 $ \xi \geqslant m $ 时有
        \begin{align*}
    ...
    \begin{align*}
        \int_{0}^{1} \abs{f''(x)\over f(x)} \dd{x} \geqslant 4
    \end{align*}
    ...
    \item 设 $ f(x) $ 在 $ [a, b] $ 上连续, 且对任意 $ x_{1}, x_{2} \in [a, b] $, $ \lambda \in (0, 1) $, 恒有 $ f(\lambda x_{1} + (1-\lambda) x_{2}) \leqslant \lambda f(x_{1}) + (1 - \lambda) f(x_{2}) $. 证明
    \begin{align*}
        f\qty(\frac{a+b}{2}) \leqslant \frac{1}{b - a}\int_{a}^{b} f(x) \dd{x} \leqslant \frac{f(a) + f(b)}{2}.
    \end{align*}
    ...
    \begin{align*}
        ab \leqslant \int_{0}^{a}f(x) \dd{x} + \int_{0}^{b} f^{-1}(y) \dd{y}.
    \end{align*}
    ...
    \end{align*}
    \item 设 $ f(x) $ 是实多项式, 且对任意实数 $ r $, 都有 $ f(r) \geqslant 0 $. 证明存在实多项式 $ g(x), h(x) $ 使得 $ f(x) = g^{2}(x) + h^{2}(x) $.   
    ...
    \item 设
    \begin{align*}
        \Q(\sqrt[n]{2}) = \set{a_{0} + a_{1}\sqrt[n]{2} + a_{2}\sqrt[n]{4} + \dots + a_{n-1}\sqrt[n]{2^{n-1}} : a_{i} \in \Q, 0\leqslant i\leqslant n-1}
    \end{align*}
    ...
    \item 设 $ A, B $ 为 $ n $ 阶方阵, 则 $ AB $ 与 $ BA $ 相似的充要条件是 $ \rank((AB)^{i}) = \rank((BA)^{i})\,(1 \leqslant i \leqslant n - 1) $.
    ...
    \item 设 $ V $ 是 $ n $ 维线性空间, 对于整数 $ k \geqslant n $, 证明存在一组向量 $ \alpha_{1}, \alpha_{2}, \dots, \alpha_{k} \in V $, 使得其中任意 $ n $ 个线性无关.
    ...
    \item 设 $ f(x) $ 在 $ [a, b] $ 上连续, $ (a, b) $ 上可导, 且 $ f(a) = f(b) $, 若 $ \abs{f'(x)} \leqslant 1 $, 证明对任意的 $ x_{1}, x_{2} \in [a, b] $, 都有
    \begin{align*}
        \abs{f(x_{1}) - f(x_{2})} \leqslant \frac{(b - a)}{2}.
    \end{align*}
    ...
    \begin{enumerate}
        ...
        \item $ V $ 必是偶数维线性空间.
    \end{enumerate}
    \item 设函数 $ f(x) \in C[a, b] $, $ f(x) $ 不恒为 $ 0 $ 并且满足 $ 0 \leqslant f(x) \leqslant M $. 证明:
    \begin{align*}
        \qty(\int_{a}^{b} f(x) \cos x \dd{x})^{2} + \qty(\int_{a}^{b} f(x) \sin x \dd{x})^{2} + \frac{M^{2}(b-a)^{4}}{12} \geqslant \qty(\int_{a}^{b} f(x) \dd{x})^{2}.
    \end{align*}
    ...
    \begin{align*}
        ...
        \end{vNiceMatrix}
    \end{align*}
    \item 设 $ f(x) $ 是定义在 $ [0, 1] $ 上的单调非增函数, 对于任意 $ a \in (0, 1) $, 证明: $ \int_{0}^{a}f(x) \dd{x} \geqslant a\int_{0}^{1}f(x) \dd{x} $.
    \item 设 $ f(x) $ 在 $ [0, 1] $ 上 Riemann 可积, 且有 $ 0 < m \leqslant f(x) \leqslant M $. 证明:
    \begin{align*}
        1 \leqslant \int_{0}^{1}f(x) \dd{x} \int_{0}^{1} \frac{1}{f(x)} \dd{x} \leqslant \frac{(M+m)^{2}}{4mM}.
    \end{align*}
\end{enumerate}
\end{document}