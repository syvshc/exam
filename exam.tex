...
\begin{enumerate}[series=exer]
    ...
    \begin{hint}
        用反证法, 假设一致连续, 再用 $ \limit{x}{+\infty}f'(x) = +\infty $ 与 Lagrange 中值定理导出矛盾.
    \end{hint}
    ...
    \begin{hint}
        ...
        \begin{align*}
            \int_{0}^{+\infty} \pdv{x}\qty(\me^{-tx}\frac{\sin x}{x}) \dd{x}
        \end{align*}
...
\end{enumerate} 
    ...
    \begin{enumerate}[resume=exer]
        ...
        \end{answer}
        \item 设 $ f $ 在 $ [0, +\infty) $ 上二阶可导, 且 $ f''(x) $ 有界, $ \limit{x}{+\infty} f(x) = 0 $, 证明: $ \limit{x}{+\infty} f'(x) = 0 $.
        \begin{hint}[法一]
            由 $ f(x) \to 0 $ 可得 $ x $ 足够大时 $ \abs{f(x)} $ 足够小, 由 Lagrange 中值定理可以得到 $ x $ 附近的某个 $ \xi $, 使得 $ f'(\xi) $ 足够小, 再利用 $ f'(x) $ 在 $ \xi $ 点的 Taylor 展开, 搭配二阶导有界的条件, 即可说明对每个足够大的 $ x $, 都有 $ \abs{f'(x)} $ 足够小, 命题得证.
        \end{hint}
        \begin{hint}[法二]
            取足够大的 $ N\in \N $, 对任意足够大的 $ x $ 将 $ f(x+1/N) $ 在 $ x $ 处 Taylor 展开, 就可以得到 $ f'(x) $ 的表达式, 再利用 $ f''(x) $ 有界, 以及 $ f(x) \to 0 $, 即可说明.
        \end{hint}
    \end{enumerate}
\end{document}