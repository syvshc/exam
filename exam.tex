...
\begin{enumerate}
    ...
    \begin{align*}
        ...
        \end{vmatrix}
    \end{align*}
    \item\label{item:反称加对角} 设 $ A $ 是 $ n $ 阶实反对称阵, $ D = \diag\qty{d_{1}, d_{2}, \dots, d_{n}} $ 是同阶的对角阵, 且 $ d_{i} > 0\,(i = 1, 2, \dots, n) $. 求证 $ \abs{A + D} > 0 $, 特别地, $ I_{n} + A $ 与 $ I_{n} - A $ 都是非异阵. 
    ...
    \item 设在数域 $ \K $ 上, 一元多项式 $ f(x) = f_{1}f_{2} $, 且 $ (f_{1}, f_{2}) = 1 $, $ V $ 是数域 $ \K $ 上的 $ n $ 维线性空间, $ \varphi $ 是 $ V $ 上的线性变换, 证明 $ \Ker f(\varphi) = \Ker f_{1}(\varphi) \oplus \Ker f_{2}(\varphi) $. 
